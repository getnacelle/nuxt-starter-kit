...
describe('Event Store', () => {
  it('sets the product in vuex store', () => {
    ...
    expect(store.state.products.products[0].product.id).toEqual(
      defaultProduct.id
    )
  })
  ...
  it('gets first variant if no variant selected', () => {
    ...
    expect(store.getters['product/currentVariant'].id).toEqual(
  ...
  it('sets the selected variant', () => {
    ...
    expect(store.state.product.selectedVariant.id).toEqual(variant.id)
  })
  ...
  it('sets the selected variant by id', () => {
    ...
    expect(store.state.product.selectedVariant.id).toEqual(variantId)
  })
})
